...
test_dataset = dataset.lmdbDataset(
    root=opt.valroot, transform=dataset.resizeNormalize((800, 32)))
...
if opt.adam:
    optimizer = optim.Adam(crnn.parameters(), lr=opt.lr,
                           betas=(opt.beta1, 0.999), weight_decay=0.005)
elif opt.adadelta:
...
def val(net, dataset, criterion, max_iter=100):
    ...
    data_loader = torch.utils.data.DataLoader(
        dataset, shuffle=False, batch_size=opt.batchSize, 
    ...
    print('Test loss: %f accuray: %f samples: %d' % (loss_avg.val(), accuracy, count))
...
def trainBatch(net, criterion, optimizer):
    ...
    preds = crnn(image)
    #print('image shape',image.size(),'preds shape',preds.size())
    preds_size = Variable(torch.IntTensor([preds.size(1)] * batch_size))
    preds = preds.permute(1,0,2)
    cost = criterion(preds, text, preds_size, length) / batch_size
    crnn.zero_grad()
