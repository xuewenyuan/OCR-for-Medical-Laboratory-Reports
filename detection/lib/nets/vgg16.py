...
class vgg16(Network):
  ...
  def build_network(self, sess, is_training=True):
    with tf.variable_scope('vgg_16', 'vgg_16'):
      ...
      if is_training:
        rois, roi_scores = self._proposal_layer(rpn_cls_prob, rpn_bbox_pred, "rois") # ind, x1, y1, x2, y2, score
      ...
      if is_training:
        fc6 = slim.dropout(fc6, keep_prob=0.5, is_training=True, scope='dropout6') #0.5
      fc7 = slim.fully_connected(fc6, 4096, scope='fc7') #4096
      if is_training:
        fc7 = slim.dropout(fc7, keep_prob=0.5, is_training=True, scope='dropout7') #0.5
  ...
  def fix_variables(self, sess, pretrained_model):
    ...
    with tf.variable_scope('Fix_VGG16') as scope:
      with tf.device("/cpu:0"):
        ...
        # fix RGB to BGR
        fc6_conv = tf.get_variable("fc6_conv", [7, 7, 512, 4096], trainable=False)#4096
        fc7_conv = tf.get_variable("fc7_conv", [1, 1, 4096, 4096], trainable=False)#4096
